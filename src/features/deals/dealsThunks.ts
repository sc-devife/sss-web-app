...
// Mirrors lib/deals.ts's getDealForTrip: a 404 (no deal yet) is a normal,
...
export const fetchDealForTrip = createAsyncThunk<Deal | null, number, { rejectValue: string }>(
  "deals/fetchDealForTrip",
  async (tripId, { rejectWithValue }) => {
    try {
      const res = await clientApi.get<Deal>(`/deals?tripId=${tripId}`, { validateStatus: (s) => s === 200 || s === 404 });
